"""
=============================================================
FUSION MODEL — Acoustic + Linguistic Dementia Detection
...
Dataset  : dementia_master_dataset.csv
           185 clips / 103 subjects (67 Dementia / 36 No Dementia
           subjects; up to 3 clips/subject)
Features : curated ac_* (acoustic) + li_*/syn_*/freq_*/sem_*/read_*
           (linguistic) -- top 30 selected via SelectKBest (MI)
Models   : XGBoost, Random Forest, SVM -- each tuned with GridSearchCV
CV       : Stratified Group 5-Fold, grouped by subject (no leakage
           between a subject's clips across folds)
=============================================================
"""
...
DATA = os.path.join(BASE, "..", "dementia_master_dataset.csv")
...
print(f"Dataset: {len(df)} clips / {df['subject'].nunique()} subjects")
print(f"  Dementia: {n_pos} | No Dementia: {n_neg}")
...
print("\n" + "=" * 60)
print("  GRID SEARCH (Stratified Group 5-Fold CV, scoring=ROC-AUC)")
print("=" * 60)

gs_results = {}
for name, (estimator, grid) in MODEL_GRIDS.items():
    print(f"\n{name} ...")
    gs = GridSearchCV(make_pipe(estimator), grid, cv=cv, scoring="roc_auc", n_jobs=-1)
...
# ── Final CV evaluation of each tuned model ──────────────────────────────
print("\n" + "=" * 60)
print("  FINAL CV EVALUATION (tuned pipelines, same CV scheme)")
print("=" * 60)
...
for name, gs in gs_results.items():
    ...
    y_pred  = cross_val_predict(tuned, X, y, cv=cv, groups=groups, method="predict")
    y_proba = cross_val_predict(tuned, X, y, cv=cv, groups=groups, method="predict_proba")[:, 1]
...
# class_weight="balanced") is known to produce predict()/predict_proba()
# that disagree on small/imbalanced folds (Platt-scaling quirk) -- it can show
# a near-perfect AUC while its predictions are degenerate (e.g. all one class).
...
print(classification_report(y, y_pred, target_names=["No Dementia", "Dementia"]))
...
ax.set_title("Dementia Fusion (Master Dataset) — Model Comparison (Tuned, CV)")
ax.set_ylabel("Score")
ax.set_ylim(0, 1)
...
ConfusionMatrixDisplay(
    confusion_matrix(y, y_pred), display_labels=["No Dementia", "Dementia"]
...
perm = permutation_importance(
    final_pipe, X, y, n_repeats=20, random_state=42, scoring="roc_auc", n_jobs=-1
)
imp_df = pd.DataFrame({
    "feature":    feat_cols,
...
ax.set_title(f"Dementia Fusion — Top 25 Feature Importances ({best_name})")
plt.tight_layout()
